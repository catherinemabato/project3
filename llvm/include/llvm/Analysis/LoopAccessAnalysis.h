...
/// Same as above, but doesn't require the element size.
inline std::pair<const SCEV *, const SCEV *> getStartAndEndForAccess(
    const Loop *Lp, const SCEV *PtrExpr, Type *AccessTy, const SCEV *MaxBECount,
    ScalarEvolution *SE,
    DenseMap<std::pair<const SCEV *, Type *>,
             std::pair<const SCEV *, const SCEV *>> *PointerBounds) {
  const SCEV *EltSize;
  return getStartAndEndForAccess(Lp, PtrExpr, AccessTy, MaxBECount, SE, EltSize,
                                 PointerBounds);
}
